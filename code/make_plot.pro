pro make_plot, zarray, str, full_str, p_mean, p_sigma,title,$
               xr=xr,$
               yr=yr,$
               tit=tit,$
               xnoedge=xnoedge,$
...
if (xmar[0] eq -99 and ymar[0] eq -99) then begin
    ploterror,x,y,yerr,/xlog,/ylog,title=title,yr=yr,xr=xr,xtickf=xtickf,ytickf=ytickf,$
...
oplot,x_mpc,nfw,color=!blue,linestyle=0
