pro plot_halofit, zarray, file, ps_file, fit_t, log_mass, log_mass_err, p_mean, p_sigma, title,$
                  makeplot=makeplot,$
                  quick_c=quick_c,$
                  pbzk=pbzk,$
...
if keyword_set(groups) then begin
   if(keyword_set(xlog)) then xr = [0.02,2] $ ;xr = [0.02,6] ;xr = [0.04,6] ;[0.03,6] 
   else xr=[0,1.5]
endif
if keyword_set(groups) then begin
   if(keyword_set(ylog)) then yr = [0.5,3000] $
   else yr = [-200,500]
endif
...
device, /cmyk, /times,font_size=14  ;/bold
    ...
    if keyword_set(groups) then begin
    make_plot_offset,zarray,str,full_str, p_mean, p_sigma, title,xr=xr,yr=yr,tit=tit,xnoedge=xnoedge,ynoedge=ynoedge,mcmc=mcmc,/groups,center=center,refcen=refcen,xlog=xlog,ylog=ylog
    endif else begin
